...
from tensorflow.keras.layers import Input, Dense, Layer, Conv2D,  Flatten, MaxPooling2D, UpSampling2D
...
class Sampling(Layer):
    ...
    def call(self, inputs):
        ...
        epsilon = tf.keras.backend.random_normal(shape=(batch,8,8,2))
        return z_mean + tf.exp(0.5 * z_log_var) * epsilon


class VAEGAN_MNISTM(object):
    def __init__(self, x_source_train, y_source_train,
        ...
        self.latent_dim_classifier = (8, 8, 48)  # 25
        self.latent_dim_decoder = (8, 8, 51)
    ...
    def build_encoder(self):
        ...
        x = MaxPooling2D(pool_size=(2, 2), strides=2)(x)
        x = Conv2D(filters=2, kernel_size=(3, 3), padding='same', activation="relu")(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=2)(x)
    ...
    def build_decoder(self):
        ...
        x = Conv2D(filters=3, kernel_size=(5, 5), padding='same', activation="sigmoid")(x)
        # x = tf.image.resize(x, [32,32], method='nearest')
    ...
    def train(self):
        ...
        def _train_step(step):
            ...
            with tf.GradientTape(persistent=True) as tape_gen:
                ...
                # concat_samples_source = tf.concat([domain_identifier_source, DIrep_source_samples], axis=3)
                # concat_samples_target = tf.concat([domain_identifier_target, DIrep_target_samples], axis=3)
