...
class VAEGAN(object):
    ...
    def build_encoder(self):
        ...
        z_mean = Dense(100, name="z_mean")(net)
        z_log_var = Dense(100, name="z_log_var")(net)
    ...
    def build_decoder(self):
        inputs = Input(self.latent_dim *2 ) # + self.input_shape)
    ...
    def g_loss(self, yhat_source, yhat_target):
        """
        DQA: 
        Why is this identical to d_loss?
        """
    ...
    def diff_loss(self, shared_rep, private_rep ): 
        #return np.linalg.norm(shared_rep.T @ private_rep)**2
        
        #return tf.norm(shared_rep.T @ private_rep, ord)
        
        shared_rep_l2_norm = np.linalg.norm(shared_rep, axis =1, keepdims =True)
        shared_rep_l2 = tf.math.divide(shared_rep, shared_rep_l2_norm + 1e-6)
        
                
        private_rep_l2_norm = np.linalg.norm(private_rep, axis =1, keepdims =True)
        private_rep_l2 = tf.math.divide(private_rep, private_rep_l2_norm + 1e-6)
        
        diff_loss = tf.math.reduce_mean((shared_rep_l2.T @ private_rep_l2)**2)
    ...
    def train(self):
        ...
        if(self.has_target_labeled):
            T_batches_unlabeled = tf.data.Dataset.from_tensor_slices((self.x_target_train_unlabeled, self.y_target_train_unlabeled)).repeat().batch(int(self.batch_size/2)).as_numpy_iterator()
            T_batches_labeled = tf.data.Dataset.from_tensor_slices((self.x_target_train_labeled, self.y_target_train_labeled)).repeat().batch(int(self.batch_size/2)).as_numpy_iterator()
        else:
            T_batches_unlabeled = tf.data.Dataset.from_tensor_slices((self.x_target_train_unlabeled, self.y_target_train_unlabeled)).repeat().batch(int(self.batch_size)).as_numpy_iterator()
        ...
        def _train_step():
            ...
            if(self.has_target_labeled):
                VAE_input_samples = tf.concat([x_batch_source, x_batch_target_labeled, x_batch_target_unlabeled], axis=0)
            else:
            ...
            with tf.GradientTape(persistent=True) as tape_gen:
                ...
                DDrep_mean_source, DDrep_log_var_source, DDrep_source = Enc(x_batch_source)
                DDrep_mean_target, DDrep_log_var_target, DDrep_target = Enc(x_batch_target_unlabeled)
               # DDrep_samples = np.array(([0] * x_batch_source.shape[0] + [1] * x_batch_target_unlabeled.shape[0])).reshape((-1, 1))
                ...
                concat_samples_source = tf.concat([DDrep_source, DIrep_source_samples], axis=1)
                concat_samples_target = tf.concat([DDrep_target, DIrep_target_samples_unlabeled], axis=1)
                ...
                kl_loss_source_value = self.kl_loss(DDrep_mean_source, DDrep_log_var_source)
                kl_loss_target_value = self.kl_loss(DDrep_mean_target, DDrep_log_var_target)
                kl_loss_value = kl_loss_source_value  +  kl_loss_target_value
                
                
                recon_loss_source_value = self.recon_loss(x_batch_source, Reconstructed_samples_source)
                recon_loss_target_value = self.recon_loss(x_batch_target_unlabeled, Reconstructed_samples_target)
                recon_loss_value =  recon_loss_source_value  + recon_loss_target_value
                
                
                diff_loss_source_value = self.diff_loss(DIrep_source_samples, DDrep_source)
                diff_loss_target_value = self.diff_loss(DIrep_target_samples_unlabeled, DDrep_target)
                diff_loss_value =  -1* (diff_loss_source_value  + diff_loss_target_value)
                
                total_loss_value =  ( kl_loss_weight * kl_loss_value +
                                       + recon_loss_weight * recon_loss_value
                                       + diff_loss_weight * diff_loss_value) / (kl_loss_weight + recon_loss_weight + diff_loss_weight)
                ...
                # Compute G loss
                g_loss_value = -1* self.d_loss(yhat_source, yhat_target)
                ...
                combined_loss_value = (g_loss_weight * g_loss_value 
                                       ...
                                       + recon_loss_weight * recon_loss_value
                                       + diff_loss_weight * diff_loss_value) / (g_loss_weight + c_loss_weight + recon_loss_weight + diff_loss_weight)
            ...
            return G, C, D, Enc, Dec, g_loss_value, c_loss_value, d_loss_value, recon_loss_value, kl_loss_value, diff_loss_value
...
#         fig, ax = plt.subplots()
        for step in range(self.nStep):
            ...
            recon_loss_value, kl_loss_value, diff_loss_value = _train_step()
            ...
            if step % 1000 == 0:
                ...
                track_loss = ('Step {} ==> G_Loss: {} C_Loss: {} D_Loss: {}  Recon_Loss: {} \n'
                              ...
                              'Acc Source: {} Acc Target: {} Acc Domain Source: {}  Acc Domain Target: {} \n' ).format(step, g_loss_value.numpy(), c_loss_value.numpy(), 
                                                                                        d_loss_value.numpy(), 
                                                                                        recon_loss_value.numpy(), kl_loss_value.numpy(),diff_loss_value.numpy(),
                                                                                        accuracy_source, accuracy_target, domain_pred_accuracy_source, domain_pred_accuracy_target)
                ...
                ## Print the original and reconstructed image  ###########
                    ...
                    #visual_bits(idx, self.x_source_test, self.y_source_test, Reconstructed_samples_0_source, Reconstructed_samples_1_source)
               
                    #print("Target test data reconstruction")
                    #visual_bits(idx, self.x_target_test, self.y_target_test, Reconstructed_samples_0_target, Reconstructed_samples_1_target)
