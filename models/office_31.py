...
class MaxDIRep(object):
    ...
    def train(self):
        # Create datasets for source and target training
        ...
        # Repeat both datasets indefinitely so we can zip them safely
        source_train_dataset = source_train_dataset.repeat()
        target_train_dataset = target_train_dataset.repeat()

        # Combine datasets using zip
        train_dataset = tf.data.Dataset.zip((source_train_dataset, target_train_dataset))

        # Determine the number of steps per epoch based on the larger dataset
        steps_per_epoch = max(
            len(self.x_source_train) // self.batch_size,
            len(self.x_target_train) // self.batch_size
        )
        ...
        for epoch in range(self.epochs):
            ...
            for step in range(steps_per_epoch):
                (source_images, class_labels), (target_images, _) = next(train_iter)
...
class MaxDIRepImagenette(object):
    def __init__(self, x_source_train, y_source_train,
                 x_target_train, y_target_train,
        ...
        self.task_optimizer = keras.optimizers.SGD(learning_rate=MyDecay(mu_0=self.lr, alpha=self.alpha),
                                                   momentum=self.momentum, nesterov=True)
        self.gen_optimizer = keras.optimizers.SGD(learning_rate=MyDecay(mu_0=self.lr / 10., alpha=self.alpha),
                                                  momentum=self.momentum, nesterov=True)
        self.disc_optimizer = keras.optimizers.SGD(learning_rate=MyDecay(mu_0=self.lr / 10., alpha=self.alpha))
        self.enc_optimizer = keras.optimizers.SGD(learning_rate=MyDecay(mu_0=self.lr / 5, alpha=self.alpha),
                                                  momentum=self.momentum, nesterov=True)
        self.dec_optimizer = keras.optimizers.SGD(learning_rate=MyDecay(mu_0=self.lr / 5, alpha=self.alpha),
                                                  momentum=self.momentum, nesterov=True)
        ...
        self.train_image_recon_loss = tf.keras.metrics.Mean()
        self.train_image_kl_loss = tf.keras.metrics.Mean()
        ...
        self.test_image_recon_loss = tf.keras.metrics.Mean()
        self.test_image_kl_loss = tf.keras.metrics.Mean()
    ...
    def train_batch(self, x_source_train, y_source_train, x_target_train, x_image_train, epoch):
        ...
        with tf.GradientTape() as task_tape, tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape, tf.GradientTape() as enc_tape, tf.GradientTape() as dec_tape:
            ...
            DIrep_source = self.generator(x_source_train)
            DIrep_target = self.generator(x_target_train)
            DIrep_image = self.generator(x_image_train)
            ...
            DDrep_mean_image, DDrep_log_var_image, DDrep_image = self.encoder(x_image_train, training=True)
            concat_samples_image = tf.concat([DIrep_image, DDrep_image], axis=3)

            x_recon_source = self.decoder(concat_samples_source, training=True)
            x_recon_target = self.decoder(concat_samples_target, training=True)
            x_recon_image = self.decoder(concat_samples_image, training=True)

            task_loss = self.loss(y_source_train, y_class_pred)
            disc_loss = self.loss(domain_labels, y_domain_pred)
            recon_loss_source = self.mse(x_source_train, x_recon_source)
            recon_loss_target = self.mse(x_target_train, x_recon_target)
            recon_loss_image = self.mse(x_image_train, x_recon_image)
            kl_loss_source = self.KL(DDrep_mean_source, DDrep_log_var_source)
            kl_loss_target = self.KL(DDrep_mean_target, DDrep_log_var_target)
            kl_loss_image = self.KL(DDrep_mean_image, DDrep_log_var_image)

            recon_loss = recon_loss_source + recon_loss_target + recon_loss_image
            kl_loss = kl_loss_source + kl_loss_target + kl_loss_image
        ...
        self.train_target_recon_loss(recon_loss_target)
        self.train_target_kl_loss(kl_loss_target)
        self.train_image_recon_loss(recon_loss_image)
        self.train_image_kl_loss(kl_loss_image)

        return

    def test_batch(self, x_source_test, y_source_test, x_target_test, y_target_test, x_image_test):
        ...
        with tf.GradientTape() as tape:
            ...
            DIrep_source = self.generator(x_source_test)
            DIrep_target = self.generator(x_target_test)
            DIrep_image = self.generator(x_image_test)
            ...
            DDrep_mean_image, DDrep_log_var_image, DDrep_image = self.encoder(x_image_test, training=False)
            concat_samples_image = tf.concat([DIrep_image, DDrep_image], axis=3)

            x_recon_source = self.decoder(concat_samples_source, training=False)
            x_recon_target = self.decoder(concat_samples_target, training=False)
            x_recon_image = self.decoder(concat_samples_image, training=False)

            task_loss = self.loss(y_source_test, y_class_pred)
            target_task_loss = self.loss(y_target_test, y_target_class_pred)
            recon_loss_source = self.mse(x_source_test, x_recon_source)
            recon_loss_target = self.mse(x_target_test, x_recon_target)
            recon_loss_image = self.mse(x_image_test, x_recon_image)
            kl_loss_source = self.KL(DDrep_mean_source, DDrep_log_var_source)
            kl_loss_target = self.KL(DDrep_mean_target, DDrep_log_var_target)
            kl_loss_image = self.KL(DDrep_mean_image, DDrep_log_var_image)
        ...
        self.test_image_recon_loss(recon_loss_image)
        self.test_image_kl_loss(kl_loss_image)
    ...
    def log(self):
        ...
        log_format = 'C_loss train: {:.4f}, Acc train : {:.2f}\n' + \
                     'D_loss train: {:.4f}, recon_loss_source:{:.4f}, kl_loss_source:{:.4f}, recon_loss_target {:.4f}, kl_loss_target {:.4f}\n' + \
                     'recon_loss_image:{:.4f}, kl_loss_image:{:.4f}\n' + \
                     'C_loss test source: {:.4f}, Acc test source: {:.2f}, recon_loss_source {:.4f}, kl_loss_source {:.4f}\n' + \
                     'C_loss test target: {:.4f}, Acc test target: {:.2f}, recon_loss_target {:.4f}, kl_loss_target {:.4f}\n' + \
                     'recon_loss_image {:.4f}, kl_loss_image {:.4f}\n'
        ...
        message = log_format.format(
            ...
            self.train_target_recon_loss.result(),
            self.train_target_kl_loss.result(),
            self.train_image_recon_loss.result(),
            self.train_image_kl_loss.result(),
            self.test_task_loss.result(),
            self.test_task_accuracy.result() * 100,
            self.test_recon_loss.result(),
            self.test_kl_loss.result(),
            ...
            self.test_target_recon_loss.result(),
            self.test_target_kl_loss.result(),
            self.test_image_recon_loss.result(),
            self.test_image_kl_loss.result())
    ...
    def reset_metrics(self, target):
        ...
        if target == 'train':
            ...
            self.train_target_recon_loss.reset_states()
            self.train_target_kl_loss.reset_states()
            self.train_image_recon_loss.reset_states()
            self.train_image_kl_loss.reset_states()
        ...
        if target == 'test':
            ...
            self.test_target_recon_loss.reset_states()
            self.test_target_kl_loss.reset_states()
            self.test_image_recon_loss.reset_states()
            self.test_image_kl_loss.reset_states()

        return

    def train(self):

        source_train_dataset = tf.data.Dataset.from_tensor_slices((self.x_source_train, self.y_source_train)).shuffle(
            len(self.y_source_train)).batch(self.batch_size)
        target_train_dataset = tf.data.Dataset.from_tensor_slices((self.x_target_train, self.y_target_train)).shuffle(
            len(self.y_target_train)).batch(self.batch_size)
        image_train_dataset = tf.data.Dataset.from_tensor_slices(self.x_image_train).shuffle(
            len(self.x_image_train)).repeat().batch(self.batch_size).as_numpy_iterator()

        source_test_dataset = tf.data.Dataset.from_tensor_slices((self.x_source_test, self.y_source_test)).batch(
            self.batch_size)
        target_test_dataset = tf.data.Dataset.from_tensor_slices((self.x_target_test, self.y_target_test)).batch(
            self.batch_size)
        image_test_dataset = tf.data.Dataset.from_tensor_slices(self.x_image_test).repeat().batch(
        ...
        for epoch in range(self.epochs):
            ...
            for (source_images, class_labels), (target_images, _) in zip(source_train_dataset, target_train_dataset):
                images = next(image_train_dataset)
                self.train_batch(source_images, class_labels, target_images, images, epoch)



            for (test_images, test_labels), (target_test_images, target_test_labels) in zip(source_test_dataset,
                                                                                            target_test_dataset):
                images = next(image_test_dataset)
                self.test_batch(test_images, test_labels, target_test_images, target_test_labels, images)
...
class MaxDIRepImagenetteBatch(object):
    def __init__(self, x_source_train, y_source_train,
                 x_target_train, y_target_train,
                 x_image_train, x_image_test,
                 x_source_test, y_source_test,
    ...
    def train(self):
        ...
        target_train_dataset = tf.data.Dataset.from_tensor_slices((self.x_target_train, self.y_target_train)).shuffle(
            len(self.y_target_train)).batch(self.batch_size)
        image_train_dataset = tf.data.Dataset.from_tensor_slices(self.x_image_train).shuffle(
            len(self.x_image_train)).repeat().batch(16).as_numpy_iterator()
